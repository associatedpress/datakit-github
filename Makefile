...
lint: ## check style with ruff
	uv run ruff check datakit_github tests
...
coverage: ## check code coverage quickly with the default Python
	uv run coverage run --source datakit_github -m pytest
	uv run coverage report -m
	uv run coverage html
	$(BROWSER) htmlcov/index.html

docs: ## generate Sphinx HTML documentation, including API docs
	rm -f docs/datakit_github.rst
	rm -f docs/modules.rst
	uv run sphinx-apidoc -o docs/ datakit_github
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
...
_bump:
	...
	uv lock && \
	git add setup.cfg pyproject.toml datakit_github/__init__.py uv.lock && \
